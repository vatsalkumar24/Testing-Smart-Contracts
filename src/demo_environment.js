...
let token0 = new web3.eth.Contract(ERC20PresetMinterPauser.abi, "", {
  data: ERC20PresetMinterPauser.bytecode,
});
let token1 = new web3.eth.Contract(ERC20PresetMinterPauser.abi, "", {
  data: ERC20PresetMinterPauser.bytecode,
});
const Arbitrager = require("../build/contracts/Arbitrager.json");
const arbitrager = new web3.eth.Contract(Arbitrager.abi, "", {
...
async function liquidity(amount0, amount1, amount2, amount3, amount4) {
  ...
  gasLimit = await token0
    .deploy({ arguments: ["Pineapple", "PNA"] })
    .estimateGas();
  receipt = await token0
    .deploy({ arguments: ["Pineapple", "PNA"] })
    .send({ from: myAccount, gas: gasLimit });
  token0.options.address = receipt._address;

  gasLimit = await token1
    .deploy({ arguments: ["Watermelon", "WTM"] })
    .estimateGas();
  ...
  gasLimit = await token0.methods
    .approve(uRouter.options.address, amount0)
    .estimateGas();
  await token0.methods
    .approve(uRouter.options.address, amount0)
    .send({ from: myAccount, gas: gasLimit });
  gasLimit = await token1.methods
    .approve(uRouter.options.address, amount1)
    .estimateGas();
  await token1.methods
    .approve(uRouter.options.address, amount1)
    .send({ from: myAccount, gas: gasLimit });

  gasLimit = await uRouter.methods
    .addLiquidity(
  ...
  console.log(
    `Uniswap ${token0Symbol}/${token1Symbol} pair created\n` +
      `Reserves: ${web3.utils.fromWei(
        amount0
      )} ${token0Symbol} | ${web3.utils.fromWei(amount1)} ${token1Symbol}\n` +
      `Price: ${(amount0 / amount1).toFixed(
        2
  ...
  gasLimit = await token1.methods
    .approve(sRouter.options.address, amount2)
    .estimateGas();
  await token0.methods
    .approve(sRouter.options.address, amount2)
    .send({ from: myAccount, gas: gasLimit });
  gasLimit = await token1.methods
    .approve(sRouter.options.address, amount3)
    .estimateGas();
  await token1.methods
    .approve(sRouter.options.address, amount3)
    .send({ from: myAccount, gas: gasLimit });
  gasLimit = await sRouter.methods
    .addLiquidity(
      token0.options.address,
      token1.options.address,
      amount2,
  ...
  console.log(
    `Sushiswap ${token0Symbol}/${token1Symbol} pair created\n` +
      `Reserves: ${web3.utils.fromWei(
        amount2
      )} ${token0Symbol} | ${web3.utils.fromWei(amount3)} ${token1Symbol}\n` +
      `Price: ${(amount2 / amount3).toFixed(
        2
      )} ${token0Symbol}/${token1Symbol}\n`
  );
}
...
async function deploy(amount0, amount1, amount2, amount3, amount4) {
  ...
  gasLimit = await arbitrager
    .deploy({ arguments: [addr0, addr1] })
    .estimateGas();
  receipt = await arbitrager
    .deploy({ arguments: [addr0, addr1] })
    .send({ from: myAccount, gas: gasLimit });
  arbitrager.options.address = receipt._address;

  console.log(
    `Arbitrager contract deployed at ${arbitrager.options.address}\n`
  );
...
if (process.argv[2] == "-a") {
  ...
  deploy(10e2, 5e2, 1e4, 10e4, 1e6).then(() => {
    process.exit(0);
  });
}
...
if (process.argv[2] == "-b") {
  ...
  deploy(1e2, 10e2, 3e4, 10e4, 1e6).then(() => {
    process.exit(0);
  });
}
